...
public class OrderService
{
	...
	public OrderResponse createOrder(OrderRequest request) throws StripeException
	{
		...
		if (artwork.isSold())
		{
			...
			throw new BusinessLogicException("Sorry, this masterpiece has already been purchased.");
		}

		// 3. Create order
		Order order = Order.builder().artwork(artwork).customerName(request.getCustomerName())
				.customerEmail(request.getCustomerEmail()).customerPhone(request.getCustomerPhone())
				.shippingAddressLine1(request.getShippingAddressLine1())
				.shippingAddressLine2(request.getShippingAddressLine2()).city(request.getCity())
				.state(request.getState()).postalCode(request.getPostalCode()).country(request.getCountry())
				.price(artwork.getPrice()).status(OrderStatus.PENDING).build();
	...
	{
		return OrderResponse.builder().id(order.getId()).artworkId(order.getArtwork().getId())
				.artworkTitle(order.getArtwork().getTitle()).artworkImageUrl(order.getArtwork().getImageUrl())
