...
public class PaymentService
{
	...
	public PaymentService(@Value("${stripe_secret_key}") String stripeSecretKey,
	...
	public String createPaymentIntent(Order order) throws StripeException
	{
		...
		PaymentService.log.info("Creating payment intent for order ID: {}, amount: ${}", order.getId(),
				order.getPrice());

		long amountInCents = order.getPrice().multiply(BigDecimal.valueOf(100)).longValueExact();

		PaymentIntentCreateParams params = PaymentIntentCreateParams.builder().setAmount(amountInCents)
				.setCurrency(currency).setDescription("Art Gallery Order #" + order.getId())
				.putMetadata("order_id", order.getId().toString())
				.putMetadata("customer_email", order.getCustomerEmail())
				.putMetadata("customer_name", order.getCustomerName()).setAutomaticPaymentMethods(
						PaymentIntentCreateParams.AutomaticPaymentMethods.builder().setEnabled(true).build())
				.build();

		RequestOptions requestOptions = RequestOptions.builder().setApiKey(stripeSecretKey)
				.setIdempotencyKey("order_" + order.getId()).build();
	...
	// This method is temporary for checking out a session to simulate a payment to
	// Stripe.
		...
		SessionCreateParams params = SessionCreateParams.builder().setMode(SessionCreateParams.Mode.PAYMENT)
				...
				.addLineItem(SessionCreateParams.LineItem.builder().setQuantity(1L)
						.setPriceData(SessionCreateParams.LineItem.PriceData.builder().setCurrency("usd")
								.setUnitAmount(order.getPrice().multiply(BigDecimal.valueOf(100)).longValueExact())
								.setProductData(SessionCreateParams.LineItem.PriceData.ProductData.builder()
										.setName("Artwork Order #" + order.getId()).build())
								.build())
						.build())
